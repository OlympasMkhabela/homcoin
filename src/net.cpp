...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Musdcoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"musdcointools.com", "52.57.110.64"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Musdcoin is probably already running."), addrBind.ToString().c_str());
        else
