...
#if defined(NDEBUG)
# error "Musdcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x44df6e324ae455ef48e3a822a5bf32a63d3f2db8f6435505c0379542cc9db294");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Musdcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Musdcoin Signed Message:\n";
...
{
    // Musdcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Musdcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Musdcoin: 3 days
static const int64 nTargetSpacing = 2 * 60; // Musdcoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Musdcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Musdcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x44df6e324ae455ef48e3a822a5bf32a63d3f2db8f6435505c0379542cc9db294");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("045584710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1504825583;
        ...
        assert(block.hashMerkleRoot == uint256("0xd3e59bad8e9ec6e5ae252e4d0ab2f753dc0488c694bbba5d09533a7eaca0cf8d"));
...
unsigned char pchMessageStart[4] = { 0xd4, 0xc4, 0xa3, 0xbc }; // Musdcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MusdcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MusdcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MusdcoinMiner(CWallet *pwallet)
{
    printf("MusdcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("musdcoin-miner");
    ...
    try { loop {
        ...
        printf("Running MusdcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MusdcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MusdcoinMiner, pwallet));
}
